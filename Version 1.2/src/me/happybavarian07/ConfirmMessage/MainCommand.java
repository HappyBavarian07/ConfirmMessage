	...
	public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
		if(cmd.getName().equalsIgnoreCase("confirmmessage") || cmd.getName().equals("cm") || cmd.getName().equalsIgnoreCase("confirmm") || cmd.getName().equalsIgnoreCase("cmessage")) {
			if(s instanceof Player) {
				Player p = (Player) s;
				if(args.length == 0) {
					p.sendMessage("§cPlease add an Argument!");
					return true;
				}
				if(p.hasPermission("ConfirmMessage.commands.confirmmessage")) {
					if(args[0].equalsIgnoreCase("Commands")) {
						if(args.length == 1) {
							...
							List<String> commands = ConfirmMessage.plugin.getConfig().getStringList("Commandlist");
							if(!commands.isEmpty()) {
								p.sendMessage("§6+----------------------------------------+");
							...
							p.sendMessage("§cToo many / few arguments");
						}
						return true;
					}
